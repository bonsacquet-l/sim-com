...
Fct_Graph_GA_GB_par_2_tiff<-function(Detection="D1",couleur_noir="couleur",
  ...
  if(couleur_noir=="couleur") {
    saveData<-file.path("Outcome","out-graph",
                        paste("GA_GB_",Detection,"_",graph_1,"_",graph_2,"_couleur_L.tiff",sep=""))
  } else {
    saveData<-file.path("Outcome","out-graph",
                        paste("GA_GB_",Detection,"_",graph_1,"_",graph_2,"_noir_blanc_L.tiff",sep=""))}
  ...
  #-- Si ce n'est pas D5
  if (Detection!="D5") {
    tiff(file=saveData, units="in", width = 10, height = 7.5, pointsize = 10,res=600)
    
    layout(matrix(c(1,2,5,6,7,3,4,5,8,9),ncol=5,byrow=TRUE),
           widths=c(2.4,2.4,0.1,2.4,2.4),heights=c(3.7,3.7))
    par(bty="o")
    ...
    title(ylab="axis1, groups A & B",line = 3,cex.lab=1.8, adj=0.5)
    ...
    boxplot(M_Grp_A_B_Axe2_G1[,colonnes],
            ...
            xlab=xlab3,
            xaxt="n",lwd=0.5,cex.lab=1.8,at=espbox)
    title(ylab="axis2, groups A & B",line = 3,cex.lab=1.8, adj=0.5)
    axis(1,at=c(0.5,6,11.5),labels = c("","C1",""),col=couleur_Axis[1],
         col.axis=couleur_Axis[1],lwd=0.7,cex.axis=1.3)
    axis(1,at=c(11.5,17.5,23.5),labels = c("","C2",""),col=couleur_Axis[2],
         col.axis=couleur_Axis[2],lwd=0.7,cex.axis=1.3)
    ...
    mtext(text =text_1, side=1, line = 5, at=par('usr')[1]-10, las=1 ,cex = 1.3)
    ...
    mtext(text =text_2, side=1, line = 5, at=par('usr')[1]-10, las=1 ,cex = 1.3)
  ...
  if (Detection=="D5") {
    ...
    mtext(text =text_1, side=1, line = 5, at=par('usr')[1]-2, las=1 ,cex = 1.3)
    ...
    mtext(text =text_2, side=1, line = 5, at=par('usr')[1]-2, las=1 ,cex = 1.3)
