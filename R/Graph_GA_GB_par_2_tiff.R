...
Fct_Graph_GA_GB_par_2_tiff<-function(Detection="D1",couleur_noir="couleur",
  ...
  if(couleur_noir=="couleur") {
    saveData<-file.path("Outcome","out-graph",
                        paste("GA_GB_",graph_1,"_",graph_2,"_",Detection,"_couleur.tiff",sep=""))
  } else {
    saveData<-file.path("Outcome","out-graph",
                        paste("GA_GB_",graph_1,"_",graph_2,"_",Detection,"_noir_blanc.tiff",sep=""))}
  ...
  if (Detection!="D5") {
    ...
    title(ylab="axis1, groups A & B",line = 2,cex.lab=1.5, adj=0.5)
    ...
    boxplot(M_Grp_A_B_Axe2_G1[,colonnes],
            ...
            xlab=xlab3,
            xaxt="n",lwd=0.5,cex.lab=1.5,at=espbox)
    title(ylab="axis2, groups A & B",line = 2,cex.lab=1.5, adj=0.5)
    axis(1,at=c(0.5,6,11.5),labels = c("","C1",""),col=couleur_Axis[1],
         col.axis=couleur_Axis[1],lwd=0.7,cex.axis=1)
    axis(1,at=c(11.5,17.5,23.5),labels = c("","C2",""),col=couleur_Axis[2],
         col.axis=couleur_Axis[2],lwd=0.7,cex.axis=1)
