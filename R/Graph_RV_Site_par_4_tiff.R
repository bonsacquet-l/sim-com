...
# de distance entre les sites         #
# pour 4 analyses                     #
# sortie en .tiff                     #
# lionel.bonsacquet                   #
#-------------------------------------#

Fct_Graph_RV_Site_par_4_tiff<-function(Detection="D1",
  ...
  if (graph_1=="ACP"){
    load(file.path("Outcome","out-regroupement","ACP","ACP_Regroup_RV_Site_good.Rdata"))
    M_Resultat_RV_G1<-M_Resultat_RV_ACP_good
    text_1<-"PCA"
  }
  
  if (graph_1=="AFC"){
    load(file.path("Outcome","out-regroupement","AFC","AFC_Regroup_RV_Site_good.Rdata"))
    M_Resultat_RV_G1<-M_Resultat_RV_AFC_good
    text_1<-"CA"
  }
  
  if (graph_1=="PCoA_bray"){
    load(file.path("Outcome","out-regroupement","PCoA","PCoA_Regroup_RV_Site_bray_good.Rdata"))
    M_Resultat_RV_G1<-M_Resultat_RV_pcoa_good
    text_1<-graph_1
  }
  
  if (graph_1=="PCoA_chao"){
    load(file.path("Outcome","out-regroupement","PCoA","PCoA_Regroup_RV_Site_chao_good.Rdata"))
    M_Resultat_RV_G1<-M_Resultat_RV_pcoa_good
    text_1<-graph_1
  }
  
  if (graph_1=="nMDS_bray"){
    load(file.path("Outcome","out-regroupement","nMDS","nMDS_Regroup_RV_Site_bray_good.Rdata"))
    M_Resultat_RV_G1<-M_Resultat_RV_nmds_good
    text_1<-graph_1
  }
  
  if (graph_1=="nMDS_chao"){
    load(file.path("Outcome","out-regroupement","nMDS","nMDS_Regroup_RV_Site_chao_good.Rdata"))
    M_Resultat_RV_G1<-M_Resultat_RV_nmds_good
    text_1<-graph_1
  }
  ...
  saveData_couleur<-file.path("Outcome","out-graph",
                              paste("RV_Site_",graph_1,"_",graph_2,"_",graph_3,"_",graph_4,"_",Detection,"_couleur.tiff",sep=""))
  saveData_noir<-file.path("Outcome","out-graph",
                           paste("RV_Site_",graph_1,"_",graph_2,"_",graph_3,"_",graph_4,"_",Detection,"_noir_blanc.tiff",sep=""))
  ...
  #-- Si ce n'est pas D5
  if (Detection!="D5") {
    tiff(file=saveData_couleur, units="in",width = 5.1, height = 7, pointsize = 11,res=300)
    ...
    title(ylab=ylab3,line = 2.5,cex.lab=2.2)
    axis(1,at=c(0.5,3.5,6.5),labels = c("","C1",""),col="black",
         col.axis="black",lwd=1,cex.axis=1.3)
    axis(1,at=c(6.5,10,13.5),labels = c("","C2",""),col="red",
         col.axis="red",lwd=1,cex.axis=1.3)
    ...
    axis(1,at=c(20.5,23.5,26.5),labels = c("","C4",""),col="orange",
         col.axis="orange",lwd=1,cex.axis=1.3)
    mtext(text ="c", side=2, line = 3, at=ylimG3[2], las=2 ,cex = 2)
    ...
    title(ylab=ylab4,line = 2.5,cex.lab=2.2)
    axis(1,at=c(0.5,3.5,6.5),labels = c("","C1",""),col="black",
         col.axis="black",lwd=1,cex.axis=1.3)
    axis(1,at=c(6.5,10,13.5),labels = c("","C2",""),col="red",
         col.axis="red",lwd=1,cex.axis=1.3)
    ...
    axis(1,at=c(20.5,23.5,26.5),labels = c("","C4",""),col="orange",
         col.axis="orange",lwd=1,cex.axis=1.3)
    mtext(text ="d", side=2, line = 3, at=ylimG4[2], las=2 ,cex = 2)
